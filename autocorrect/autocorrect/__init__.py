...
from suffix_tries import SuffixTrie
...
def load_suffx_tree(file):
    str = ""
    for word in file:
        str += word + " "
    return SuffixTrie(str)
...
class Speller:
    def __init__(
        ...
        # self.vocab = load_suffx_tree(load_from_tar(lang))
    ...
    def existing(self, words):
        """{'the', 'teh'} => {'the'}"""
        # return {word for word in words if self.vocab.search(word)}
